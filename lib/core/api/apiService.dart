...
class Api {
  // Singleton Dio instance with optimized configuration
  ...
  static Dio _createDio() {
    ...
    dio.options = BaseOptions(
      ...
      maxRedirects: 3,
      headers: {
        'Accept-Encoding': null, // Disable automatic compression headers
        'Connection': null, // Disable automatic connection headers
      },
    );
    
    // Add curl logger only in debug mode for performance
      ...
      if (response.data[ApiURL.errorKey]) {
        ...
        throw ApiException(
            SettingsRepository().getCurrentAppLanguage().code != null &&
                    SettingsRepository().getCurrentAppLanguage().code !=
                        englishLangCode
                ? response.data[ApiURL.languageMessageKey]??response.data[ApiURL.messageKey]
                : response.data[ApiURL.messageKey].toString());
      }
      if (SettingsRepository().getCurrentAppLanguage().code != null &&
          SettingsRepository().getCurrentAppLanguage().code !=
