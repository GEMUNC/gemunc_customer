...
class FavoriteButton extends StatelessWidget {
        ...
        return BlocConsumer<RemoveFavoriteCubit, RemoveFavoriteState>(
          ...
          if (state is RemoveFavoriteSuccess) {
            if (product != null && product!.id == state.id) {
              ...
              // Update all cart products with same product ID
              _updateCartProductsFavoriteStatus(context, product!.id!, false);
            }
            ...
            listener: (context, state) {
              if (state is AddFavoriteSuccess) {
                if (product != null && product!.id == state.id) {
                  ...
                  // Update all cart products with same product ID
                  _updateCartProductsFavoriteStatus(context, product!.id!, true);
                }
                if (seller != null && seller!.sellerId == state.id) {
  ...
  // Helper method to update cart products favorite status
  void _updateCartProductsFavoriteStatus(BuildContext context, int productId, bool isFavorite) {
    try {
      final cartCubit = context.read<GetUserCartCubit>();
      final currentState = cartCubit.state;
      
      if (currentState is GetUserCartFetchSuccess) {
        ...
        if (cart.cartProducts != null) {
          for (var cartProduct in cart.cartProducts!) {
            if (cartProduct.productDetails != null && cartProduct.productDetails!.isNotEmpty) {
              final productDetail = cartProduct.productDetails![0];
              if (productDetail.id == productId) {
                productDetail.setFavoriteProduct(isFavorite);
                hasUpdates = true;
              }
            }
          }
        ...
        // Update saved for later products
        if (cart.saveForLaterProducts != null) {
          for (var cartProduct in cart.saveForLaterProducts!) {
            if (cartProduct.productDetails != null && cartProduct.productDetails!.isNotEmpty) {
              final productDetail = cartProduct.productDetails![0];
              if (productDetail.id == productId) {
                productDetail.setFavoriteProduct(isFavorite);
        ...
        // If any updates were made, emit the updated cart state
        if (hasUpdates) {
          cartCubit.emitSuccessState(cart);
        }
      }
    } catch (e) {
      // Silently handle any errors to avoid disrupting the user experience
      // This ensures the favorite functionality continues to work even if cart updates fail
    }
  }
}
