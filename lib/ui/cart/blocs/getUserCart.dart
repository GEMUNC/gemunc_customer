...
class GetUserCartCubit extends Cubit<GetUserCartState> {
  ...
  setErrorMessage(int productId, String errorMessage, {int? variantId}) {
    if (state is GetUserCartFetchSuccess) {
      int index = -1;
      
      // First try to match by variant ID if provided (for products with variants)
      if (variantId != null) {
        index = (state as GetUserCartFetchSuccess).cart.cartProducts!.indexWhere(
          (product) => product.productVariantId == variantId,
        );
      }
      
      // If no match found by variant ID or no variant ID provided, try matching by product ID
      if (index == -1) {
        index = (state as GetUserCartFetchSuccess).cart.cartProducts!.indexWhere(
