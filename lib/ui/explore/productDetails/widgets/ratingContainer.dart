...
class RatingContainerState extends State<RatingContainer>
  ...
  late ProductRatingCubit _reviewListCubit;

  @override
  void initState() {
    super.initState();
    _reviewListCubit = ProductRatingCubit();
    getRatingData();
    getReviewListData();
  ...
  getReviewListData() {
    Future.delayed(Duration.zero, () {
      _reviewListCubit.resetState();
      _reviewListCubit.getProductRating(
          params: {
            ApiURL.productIdApiKey: widget.product.id,
            ApiURL.limitApiKey: widget.isFullScreen ? limit : 3,
            ApiURL.offsetApiKey: 0,
  ...
  loadMoreReviewListRatings() {
    _reviewListCubit.loadMore(
        params: {
          ApiURL.productIdApiKey: widget.product.id,
          if (_selectedStar != 0) ApiURL.ratingApiKey: _selectedStar
        },
  ...
  Widget buildRatingWidget(ProductRatingState state) {
    if (state is ProductRatingSuccess) {
  ...
  buildReviewListWidget() {
    return BlocBuilder<ProductRatingCubit, ProductRatingState>(
      bloc: _reviewListCubit,
      builder: (context, state) {
        if (state is ProductRatingSuccess) {
          return buildReviewList(state);
        } else if (state is ProductRatingFailure) {
          return Center(
    ...
    return Column(children: <Widget>[
                      ...
                      itemBuilder: (context, index) {
                        if (_reviewListCubit.hasMore()) {
                          if (index ==
                            ...
                            if (_reviewListCubit.fetchMoreError()) {
                              return Center(
                                child: CustomTextButton(
                                    buttonTextKey: retryKey,
                                    onTapButton: () {
      ...
      Column(
        children: [
          ...
          GestureDetector(
            onTap: () => Utils.navigateToScreen(context, Routes.allReviewScreen,
                arguments: {
                  'product': widget.product,
                  'isComboProduct': widget.isComboProduct,
                  'isFullScreen': true,
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(
                children: <Widget>[
                  CustomTextContainer(
                    textKey: _selectedStar == 0
                        ? allReviewsKey
                        : '${_selectedStar.toString()}',
                    style: Theme.of(context).textTheme.bodySmall!.copyWith(
                        color: Theme.of(context)
                            .colorScheme
                            .secondary
