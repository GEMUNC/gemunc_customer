...
class ProductFiltersScreen extends StatefulWidget {
  ...
  final List<Category>? categories; // Pre-fetched categories data
  final List<Brand>? brands; // Pre-fetched brands data
  ...
  static Widget getRouteInstance() {
    ...
    return MultiBlocProvider(
      ...
      child: ProductFiltersScreen(
        ...
        brands: arguments['brands'] as List<Brand>?,
      ),
    );
  }
...
class _ProductFiltersScreenState extends State<ProductFiltersScreen> {
  ...
  late final TextEditingController minPriceTextEditingController =
    ...
    text: widget.selctedMinPrice.isNotEmpty 
        ? widget.selctedMinPrice 
        : widget.minPrice.toStringAsFixed(2),
  );
  late final TextEditingController maxPriceTextEditingController =
      TextEditingController(
    text: widget.selctedMaxPrice.isNotEmpty 
        ? widget.selctedMaxPrice 
        : widget.maxPrice.toStringAsFixed(2),
  );

  bool hasMadeAnyChanges = false;
  bool hasCustomPriceValues = false;
  ...
  void initState() {
    ...
    ///[Check if custom price values are already set]
    _checkCustomPriceValues();

    ///[Use pre-fetched categories and brands data if available, otherwise fetch from API]
    if (widget.category == null && widget.categories == null) {
      context.read<CategoryCubit>().fetchCategories(
          storeId: context.read<StoresCubit>().getDefaultStore().id!,
          categoryIds: widget.categoryIds);
    } else if (widget.categories != null) {
      // Use pre-fetched categories data
      context.read<CategoryCubit>().setCategoriesDirectly(widget.categories!);
    }
    
    if (widget.brandId == null && widget.brands == null) {
      context.read<BrandsCubit>().getBrands(
          storeId: context.read<StoresCubit>().getDefaultStore().id!,
          brandIds: widget.brandIds);
    } else if (widget.brands != null) {
      // Use pre-fetched brands data
      context.read<BrandsCubit>().setBrandsDirectly(widget.brands!);
    }
  }
  ...
  ///[Check if custom price values are set]
  void _checkCustomPriceValues() {
    final currentMinPrice = minPriceTextEditingController.text.trim();
    final currentMaxPrice = maxPriceTextEditingController.text.trim();
    final defaultMinPrice = widget.minPrice.toStringAsFixed(2);
    final defaultMaxPrice = widget.maxPrice.toStringAsFixed(2);
    
    hasCustomPriceValues = currentMinPrice != defaultMinPrice || 
                          currentMaxPrice != defaultMaxPrice;
  }

  ///[Get the display count for a filter attribute]
  int _getFilterAttributeDisplayCount(SelectedFilterAttribute filterAttribute) {
    if (filterAttribute.attributeName == priceKey) {
      // For price, show count based on custom values or selected price ranges
      if (hasCustomPriceValues) {
        return 1; // Show 1 when custom price values are set
      } else {
        return filterAttribute.selectedIds.length; // Show actual selected price ranges
      }
    }
    return filterAttribute.selectedIds.length;
  }
            ...
            Center(
              child: Padding(
                ...
                child: Row(children: [
                  ...
                  _getFilterAttributeDisplayCount(filterAttribute) == 0
                      ? const SizedBox()
                      : CustomTextContainer(
                          textKey:
                              _getFilterAttributeDisplayCount(filterAttribute).toString(),
      ...
      if (selectedFilterAttribute.attributeName == priceKey) {
        return PriceFilterView(
          ...
          onChangedMinMaxPrice: (minPrice, maxPrice) {
            // Mark that changes have been made to enable the Apply button
            if (!hasMadeAnyChanges) {
              hasMadeAnyChanges = true;
            }
            
            // When user types in text fields, clear any selected price range
            // because custom text input should be independent of predefined ranges
            if (minPrice != null || maxPrice != null) {
              // Find and clear any selected price range when user uses custom input
              for (var filterAttribute in filterAttributes) {
                if (filterAttribute.attributeName == priceKey) {
                  filterAttribute.selectedIds.clear();
                  break;
                }
              }
            }
            
            // Update custom price values flag
            _checkCustomPriceValues();
          ...
          onTapPriceRange: (id) {
            ...
            // When user selects a predefined price range, clear custom text fields
            // Reset to default values to avoid confusion
            minPriceTextEditingController.text = widget.minPrice.toStringAsFixed(2);
            maxPriceTextEditingController.text = widget.maxPrice.toStringAsFixed(2);
            
            // Update custom price values flag after resetting text fields
            _checkCustomPriceValues();
          },
        );
      }
