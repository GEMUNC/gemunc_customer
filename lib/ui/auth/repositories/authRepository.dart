...
class AuthRepository {
  ...
  Future<({bool hasError, String message})> verifyCredentials({required Map<String, dynamic> params}) async {
    try {
      final result = await Api.post(
          body: params, url: ApiURL.verifyCredentials, useAuthToken: false);

      return (
        hasError: (result[ApiURL.errorKey] ?? false) as bool,
        message: (result[ApiURL.messageKey] ?? '') as String
      );
    } catch (e) {
      if (e is ApiException) {
        throw ApiException(e.toString());
  ...
  Future<User?> signInWithGoogle() async {
    try {
      ...
      debugPrint('Google Sign-In error2: $e');
      if(e.toString() == appleLoginCancelledErrorMsg){
        throw ApiException(appleLoginCancelledErrorMsg);
      }
      else{
      throw ApiException(errorInSiginnGoogleErrorMessageKey);}
